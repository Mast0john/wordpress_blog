 ...
 */
 ...
 /* Template_Parse_Content class is used to parse the data from content.xml file. After parsing xml data is converted to different arrays
...
/* Template_Import_Content class is used to process the parsed data. start_import() method is used to save the data into
 the database, and if the data is successfully saved into the database then it will reture the true, else return false. */
class Template_Import_Content
{
//Theme_Content_Import
    ...
    public function xml_parser(){
        $this->parser = new Template_Parse_Content(get_template_directory() . '/content/content.xml');
        // parses content.xml
    ...
    public function start_import($filteredContent)
    {
      ...
      /*  if ($contactus_info) {
        ...
        // if title set from Template , update it
        if ($title) {
           update_option('blogname', $title);
        }
        
        // if slogan set from Template , update it
        if ($slogan) {
	...
	{	
		...
		{	
			...
			$i = $i+1;	
			$j += 1;
			$properties[] = '<div class="form-group row"> ';
		...
		$properties[] = '<div class="form-group row"><label class="col-md-4 control-label"> ' .$attr['msg_name']. '</label>';
		...
		$properties[] = '<div class="form-group row"><label class="col-md-4 control-label"> File </label>';
		...
		$properties[] = '<div class="form-group row"><div class="col-md-8 col-md-offset-4 offset-md-4 ">'; // Merged Bootstrap 3 & 4 classes.
...
function tt_link_src($match) {
 ...
 function tt_video_src($match)  {
    ...
    global $wp_filesystem;
        ...
        $this->Template_import_content_image($ttr_media);
         return $result;
    }
...
// function created to start importing content images.
  	private function Template_import_content_image($ttr_media)
    {
		$parent_post_id = 0;
...
function Template_parse_xml()
{
    $tt_content_importer = new Template_Import_Content();
    $result = $tt_content_importer->xml_parser();
    return $result;
}

// instance created and start the importing process.
function Template_import_start($filteredContent)
{
    $tt_content_importer = new Template_Import_Content();
